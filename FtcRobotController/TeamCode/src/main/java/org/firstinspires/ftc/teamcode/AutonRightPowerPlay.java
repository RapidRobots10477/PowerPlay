 ...
 *
 *      "Proportional Gain" is a constant to set the "strength" of the steering response.
 *
 */

@Autonomous(name="Auton RIGHT PowerPlay Medium Pole", group="Medium")


public class AutonRightPowerPlay extends LinearOpMode {
    ...
    private float signalConfidence = 0.60f;
   ...
   // private DcMotor         armLiftDrive     = null;\
    private DcMotor         armRightDrive    =null;
    private DcMotor         armLeftDrive      =null;
    ...
    static final int CONE_PICK_UP        = -800;
    ...
    public void runOpMode() {
        ...
        if (opModeInInit()) {
            ...
            telemetry.addData(">", "ArmLeftDrive Encoder = ", armLeftDrive.getCurrentPosition());
             telemetry.addData(">", "ArmRightDrive Encoder = ", armRightDrive.getCurrentPosition());
        ...
        moveArmDown(RELEASE_MEDIUM_POLE);
        //sleep(2000);
        openClaw();
        holdHeading(TURN_SPEED,  -45.0, 0.5);

        driveStraight(DRIVE_SPEED, 10.0, -45.0);
        moveArmDown(CONE_PICK_UP);
        ...
        moveArmUp(CONE_PICK_UP);
        driveStraight(DRIVE_SPEED, 4.0, -90);
        moveArmDown(-845);
        ...
        holdHeading(TURN_SPEED,  -130.0, 0.5);
        driveStraight(DRIVE_SPEED, 11.0, -130);
        moveArmDown(-850);
        openClaw();
    ...
    private void moveArmUp(int holdPosition) {
        armLeftDrive.setTargetPosition(holdPosition);
        while (armLeftDrive.getCurrentPosition() > holdPosition) {
            armLeftDrive.setPower(ARM_UP_POWER);
            armRightDrive.setPower(ARM_UP_POWER);
            telemetry.addData("ArmPosition>", armLeftDrive.getCurrentPosition());
            telemetry.update();
        }
        armLeftDrive.setPower(0);
        armRightDrive.setPower(0);
         
        armLeftDrive.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
        armRightDrive.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
    }
    ...
    private void moveArmDown(int holdPosition) {
        while (holdPosition > armLeftDrive.getCurrentPosition()) {
            armLeftDrive.setPower(ARM_DOWN_POWER);
            armRightDrive.setPower(ARM_DOWN_POWER);
    ...
    private void initVuforia() {
        ...
        webcam1 = hardwareMap.get(WebcamName.class, "Webcam1");   // robot left webcam
        webcam2 = hardwareMap.get(WebcamName.class, "Webcam2");   // robot right webcam
    ...
    private void performMotorConfig(){
        ...
        armRightDrive   = hardwareMap.get(DcMotor.class, "right_arm");
        armLeftDrive    = hardwareMap.get(DcMotor.class, "left_arm");
        ...
        armLeftDrive.setDirection(DcMotor.Direction.FORWARD);
        armRightDrive.setDirection(DcMotor.Direction.REVERSE);
        ...
        //armLiftDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
         armRightDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        armLeftDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        ...
        armRightDrive.setMode(DcMotor.RunMode.RESET_ENCODERS);
        armLeftDrive.setMode(DcMotor.RunMode.RESET_ENCODERS);
        //armLiftDrive.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
        armRightDrive.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
        armLeftDrive.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
