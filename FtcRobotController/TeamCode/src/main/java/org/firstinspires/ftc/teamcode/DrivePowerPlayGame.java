...
public class DrivePowerPlayGame extends OpMode {
    ...
    static final int HIGH_POLE           = -2850;
    static final int MEDIUM_POLE         = -1950;
    ...
    private void moveArmUpThread(int holdPosition) {
        try {
    ...
    private void moveArmDownThread(int holdPosition) {
        try {
            if (thread != null && thread.isAlive()) {
                try {
                    thread.interrupt();
